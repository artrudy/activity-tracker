...
document.querySelector("#createUser")?.addEventListener("click", (e) => {
    e.preventDefault();
    const loginMenu = document.querySelector("#userLogin");
    if (loginMenu)
    ...
    function validateLogin() {
        ...
        if (dataFromLocalStorage) {
            ...
            if (password === passwordFromStorage) {
                const currentUser = encodeURIComponent(JSON.parse(dataFromLocalStorage).name);
...
function displayUserActivities(user) {
